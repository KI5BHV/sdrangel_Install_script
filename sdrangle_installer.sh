...
sudo apt-get -y install git cmake g++ pkg-config autoconf automake \
libtool libfftw3-dev libusb-1.0-0-dev libusb-dev qt5-default qtbase5-dev \
qtchooser libqt5multimedia5-plugins qtmultimedia5-dev libqt5websockets5-dev \
qttools5-dev qttools5-dev-tools libqt5opengl5-dev qtbase5-dev libboost-all-dev \
libasound2-dev pulseaudio libopencv-dev libxml2-dev bison flex ffmpeg \ 
libavcodec-dev libavformat-dev libopus-dev
...
cmake \
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/dsdcc \
-DUSE_MBELIB=ON -DLIBMBE_INCLUDE_DIR=/opt/install/mbelib/include \
...
cmake \
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/codec2 ..
make -j $PROC install
...
cmake \
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/libmirisdr ..
make -j $PROC install
...
-DCMAKE_INSTALL_PREFIX=/opt/install/librtlsdr ..
make -j $PROC install
...
cmake \
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/libiio -DINSTALL_UDEV_RULE=OFF ..
make -j $PROC install
...
cmake \
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/libhackrf \
...
cmake \
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/LimeSuite ..
make -j $PROC install
...
cmake \
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/libairspyhf ..
make -j $PROC install
...
cmake \
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/libperseus ..
make
make - j $PROC install
...
cmake \
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/xtrx-images -DENABLE_SOAPY=NO ..
make
make -j $PROC install
...
mkdir build; cd build
cmake \
-DCMAKE_INSTALL_PREFIX=/opt/install/SoapySDR ..
make -j $PROC install
...
cmake \
-DCMAKE_INSTALL_PREFIX=/opt/install/SoapySDR  \
-DRTLSDR_INCLUDE_DIR=/opt/install/librtlsdr/include \
-DRTLSDR_LIBRARY=/opt/install/librtlsdr/lib/librtlsdr.so \
-DSOAPY_SDR_INCLUDE_DIR=/opt/install/SoapySDR/include \
-DSOAPY_SDR_LIBRARY=/opt/install/SoapySDR/lib/libSoapySDR.so ..
make -j $PROC install
...
cmake \
-DCMAKE_INSTALL_PREFIX=/opt/install/SoapySDR \
-DLIBHACKRF_INCLUDE_DIR=/opt/install/libhackrf/include/libhackrf \
-DLIBHACKRF_LIBRARY=/opt/install/libhackrf/lib/libhackrf.so \
-DSOAPY_SDR_INCLUDE_DIR=/opt/install/SoapySDR/include \
-DSOAPY_SDR_LIBRARY=/opt/install/SoapySDR/lib/libSoapySDR.so ..
make -j $PROC install
...
-Wno-dev \
-DCMAKE_INSTALL_PREFIX=/opt/install/LimeSuite \
-DCMAKE_PREFIX_PATH=/opt/install/SoapySDR ..
make -j $PROC install
...
cmake \
-DCMAKE_INSTALL_PREFIX=/opt/install/SoapySDR \
-DLIBSDRPLAY_INCLUDE_DIRS=/opt/install/libsdrplay/include \
-DLIBSDRPLAY_LIBRARIES=/opt/install/libsdrplay/lib/libmirsdrapi-rsp.so \
-DSOAPY_SDR_INCLUDE_DIR=/opt/install/SoapySDR/include \
-DSOAPY_SDR_LIBRARY=/opt/install/SoapySDR/lib/libSoapySDR.so ..
make -j $PROC install
